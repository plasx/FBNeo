...
INT32 MakeScreenShot()
{
    ...
    _stprintf(szSShotName,"%s%s-%.2d-%.2d-%.2d%.2d%.2d.png", SSHOT_DIRECTORY, BurnDrvGetTextA(DRV_NAME), tmTime->tm_mon + 1, tmTime->tm_mday, tmTime->tm_hour, tmTime->tm_min, tmTime->tm_sec);
	//_stprintf(szTime,"%.2d-%.2d-%.2d %.2d:%.2d:%.2d", tmTime->tm_mon + 1, tmTime->tm_mday, tmTime->tm_year, tmTime->tm_hour, tmTime->tm_min, tmTime->tm_sec);
	_stprintf(szTime, "%s", asctime(tmTime));
#if defined(BUILD_SDL2) && !defined(SDL_WINDOWS)
...
#ifdef _UNICODE
	_stprintf(szAuthor, APP_TITLE " v%.20ls", szAppBurnVer);
#else
	_stprintf(szAuthor, APP_TITLE " v%.20s", szAppBurnVer);
#endif
	_stprintf(szDescription, "Screenshot of %s", DecorateGameName(nBurnDrvActive));
	_stprintf(szCopyright, "%s %s", BurnDrvGetTextA(DRV_DATE), BurnDrvGetTextA(DRV_MANUFACTURER));
#ifdef _UNICODE
	_stprintf(szSoftware, APP_TITLE " v%.20ls using LibSPNG v%d.%d.%d", szAppBurnVer, SPNG_VERSION_MAJOR, SPNG_VERSION_MINOR, SPNG_VERSION_PATCH);
#else
	_stprintf(szSoftware, APP_TITLE " v%.20s using LibSPNG v%d.%d.%d", szAppBurnVer, SPNG_VERSION_MAJOR, SPNG_VERSION_MINOR, SPNG_VERSION_PATCH);
#endif
	_stprintf(szSource, "%s video game hardware", BurnDrvGetTextA(DRV_SYSTEM));
