...
static int alumac(char *buffer, int dest, int op)
{
	...
	if (opstring[0] == '!')
		return snprintf(buffer, 64, opstring + 1, dst, yop, xop);
	else
		return snprintf(buffer, 64, opstring, dst, xop, yop);
}
...
static int aluconst(char *buffer, int dest, int op)
{
	...
	opstring = alumac_op[opindex][((op >> 11) & 3) == 3];
	if (opstring[0] == '!')
		return snprintf(buffer, 64, opstring + 1, dst, cval, xop);
	else
		return snprintf(buffer, 64, opstring, dst, xop, cval);
}
	...
	switch ((op >> 16) & 0xff)
	{
		...
		case 0x01:
			...
			if ((op & 0x008000) == 0x000000)
				buffer += snprintf(buffer, 64, "%s = IO($%X)", reg_grp[0][op & 15], (op >> 4) & 0x7ff);
			else
				buffer += snprintf(buffer, 64, "IO($%X) = %s", (op >> 4) & 0x7ff, reg_grp[0][op & 15]);
			break;
		case 0x02:
			/* 00000010 0000xxxx xxxxxxxx  modify flag out */
			if ((op & 0x00f000) == 0x000000)
			{
				buffer += snprintf(buffer, 64, "%s", condition[op & 15]);
				buffer += snprintf(buffer, 64, flag_change[(op >> 4) & 3], "FLAG_OUT");
				buffer += snprintf(buffer, 64, flag_change[(op >> 6) & 3], "FL0");
				buffer += snprintf(buffer, 64, flag_change[(op >> 8) & 3], "FL1");
				buffer += snprintf(buffer, 64, flag_change[(op >> 10) & 3], "FL2");
			}
			...
			else if ((op & 0x00fff0) == 0x008000)
			{
				...
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x03:
			/* 00000011 xxxxxxxx xxxxxxxx  call or jump on flag in */
			if (op & 2)
				buffer += snprintf(buffer, 64, "%s", "IF FLAG_IN ");
			else
				buffer += snprintf(buffer, 64, "%s", "IF NOT FLAG_IN ");
			if (op & 1)
			{
				buffer += snprintf(buffer, 64, "%s", "CALL ");
				dasmflags = DASMFLAG_STEP_OVER;
			}
			else
				buffer += snprintf(buffer, 64, "%s", "JUMP ");
			temp = ((op >> 4) & 0x0fff) | ((op << 10) & 0x3000);
			buffer += snprintf(buffer, 64, "$%04X", temp);
			break;
		case 0x04:
			...
			if ((op & 0x00ffe0) == 0x000000)
			{
				if (op & 0x000010)
				{
					buffer += snprintf(buffer, 64, "%s", "POP PC ");
					dasmflags = DASMFLAG_STEP_OUT;
				}
				if (op & 0x000008) buffer += snprintf(buffer, 64, "%s", "POP LOOP ");
				if (op & 0x000004) buffer += snprintf(buffer, 64, "%s", "POP CNTR ");
				if ((op & 0x000003) == 0x000002) buffer += snprintf(buffer, 64, "%s", "PUSH STAT ");
				else if ((op & 0x000003) == 0x000003) buffer += snprintf(buffer, 64, "%s", "POP STAT ");
			}
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x05:
			...
			if ((op & 0x00ffff) == 0x000000)
				buffer += snprintf(buffer, 64, "%s", "IF MV SAT MR");
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x06:
			...
			if ((op & 0x00e0ff) == 0x000000)
				buffer += snprintf(buffer, 64, "DIVS %s,%s", alu_yop[(op >> 11) & 3], alu_xop[(op >> 8) & 7]);
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x07:
			...
			if ((op & 0x00f8ff) == 0x001000)
				buffer += snprintf(buffer, 64, "DIVQ %s", alu_xop[(op >> 8) & 7]);
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x08:
		...
		case 0x09:
			...
			{
				temp = (op >> 2) & 4;
				buffer += snprintf(buffer, 64, "MODIFY (I%d,M%d)", temp + ((op >> 2) & 3), temp + (op & 3));
			}
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x0a:
			...
			if ((op & 0x00ffe0) == 0x000000)
			{
				buffer += snprintf(buffer, 64, "%s", condition[op & 15]);
				if (op & 0x000010)
					buffer += snprintf(buffer, 64, "%s", "RTI");
				else
					buffer += snprintf(buffer, 64, "%s", "RTS");
				dasmflags = DASMFLAG_STEP_OUT;
			}
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
		...
		case 0x0b:
			...
			if ((op & 0x00ff00) == 0x000000)
			{
				...
				if (op & 0x000010)
				{
					buffer += snprintf(buffer, 64, "CALL (I%d)", 4 + ((op >> 6) & 3));
					dasmflags = DASMFLAG_STEP_OVER;
				}
				else
					buffer += snprintf(buffer, 64, "JUMP (I%d)", 4 + ((op >> 6) & 3));
			}
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x0c:
			...
			buffer += snprintf(buffer, 64, mode_change[(op >> 4) & 3], "SEC_REG");
			buffer += snprintf(buffer, 64, mode_change[(op >> 6) & 3], "BIT_REV");
			buffer += snprintf(buffer, 64, mode_change[(op >> 8) & 3], "AV_LATCH");
			buffer += snprintf(buffer, 64, mode_change[(op >> 10) & 3], "AR_SAT");
			buffer += snprintf(buffer, 64, mode_change[(op >> 12) & 3], "M_MODE");
			buffer += snprintf(buffer, 64, mode_change[(op >> 14) & 3], "TIMER");
			buffer += snprintf(buffer, 64, mode_change[(op >> 2) & 3], "G_MODE");
			break;
		case 0x0d:
			/* 00001101 0000xxxx xxxxxxxx  internal data move */
			if ((op & 0x00f000) == 0x000000)
				buffer += snprintf(buffer, 64, "%s = %s", reg_grp[(op >> 10) & 3][(op >> 4) & 15], reg_grp[(op >> 8) & 3][op & 15]);
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x0e:
			...
			{
				buffer += snprintf(buffer, 64, "%s", condition[op & 15]);
				buffer += snprintf(buffer, 64, shift_op[(op >> 11) & 15], shift_xop[(op >> 8) & 7]);
			}
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x0f:
			...
			if ((op & 0x008000) == 0x000000)
				buffer += snprintf(buffer, 64, shift_by_op[(op >> 11) & 15], shift_xop[(op >> 8) & 7], (signed char)op);
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x10:
			...
			{
				buffer += snprintf(buffer, 64, shift_op[(op >> 11) & 15], shift_xop[(op >> 8) & 7]);
				buffer += snprintf(buffer, 64, ", %s = %s", reg_grp[0][(op >> 4) & 15], reg_grp[0][op & 15]);
			}
			else
				buffer += snprintf(buffer, 64, "??? (%06X)", op);
			break;
		case 0x11:
			...
			buffer += snprintf(buffer, 64, shift_op[(op >> 11) & 15], shift_xop[(op >> 8) & 7]);
			if (op & 0x008000)
				buffer += snprintf(buffer, 64, ", PM(I%d,M%d) = %s", 4 + ((op >> 2) & 3), 4 + (op & 3), reg_grp[0][(op >> 4) & 15]);
			else
				buffer += snprintf(buffer, 64, ", %s = PM(I%d,M%d)", reg_grp[0][(op >> 4) & 15], 4 + ((op >> 2) & 3), 4 + (op & 3));
			break;
		case 0x12: case 0x13:
			...
			temp = (op >> 14) & 4;
			if (op & 0x008000)
				buffer += snprintf(buffer, 64, ", DM(I%d,M%d) = %s", temp + ((op >> 2) & 3), temp + (op & 3), reg_grp[0][(op >> 4) & 15]);
			else
				buffer += snprintf(buffer, 64, ", %s = DM(I%d,M%d)", reg_grp[0][(op >> 4) & 15], temp + ((op >> 2) & 3), temp + (op & 3));
			break;
		case 0x14: case 0x15: case 0x16: case 0x17:
			/* 000101xx xxxxxxxx xxxxxxxx  do until */
			buffer += snprintf(buffer, 64, "DO $%04X UNTIL %s", (op >> 4) & 0x3fff, do_condition[op & 15]);
			break;
		case 0x18: case 0x19: case 0x1a: case 0x1b: case 0x1c: case 0x1d: case 0x1e: case 0x1f:
			/* 00011xxx xxxxxxxx xxxxxxxx  conditional jump (immediate addr) */
			if (op & 0x040000)
			{
				buffer += snprintf(buffer, 64, "%sCALL $%04X", condition[op & 15], (op >> 4) & 0x3fff);
				dasmflags = DASMFLAG_STEP_OVER;
			}
			else
				buffer += snprintf(buffer, 64, "%sJUMP $%04X", condition[op & 15], (op >> 4) & 0x3fff);
			break;
		...
		case 0x38: case 0x39: case 0x3a: case 0x3b: case 0x3c: case 0x3d: case 0x3e: case 0x3f:
			...
			buffer += snprintf(buffer, 64, "%s = $%04X", reg_grp[(op >> 18) & 3][op & 15], (op >> 4) & 0x3fff);
			break;
		case 0x40: case 0x41: case 0x42: case 0x43: case 0x44: case 0x45: case 0x46: case 0x47:
		case 0x48: case 0x49: case 0x4a: case 0x4b: case 0x4c: case 0x4d: case 0x4e: case 0x4f:
			/* 0100xxxx xxxxxxxx xxxxxxxx  load data register immediate */
			buffer += snprintf(buffer, 64, "%s = $%04X", reg_grp[0][op & 15], (op >> 4) & 0xffff);
			break;
		...
		case 0x98: case 0x99: case 0x9a: case 0x9b: case 0x9c: case 0x9d: case 0x9e: case 0x9f:
			...
			if (op & 0x100000)
				buffer += snprintf(buffer, 64, "DM($%04X) = %s", (op >> 4) & 0x3fff, reg_grp[(op >> 18) & 3][op & 15]);
			else
				buffer += snprintf(buffer, 64, "%s = DM($%04X)", reg_grp[(op >> 18) & 3][op & 15], (op >> 4) & 0x3fff);
			break;
		...
		case 0xb8: case 0xb9: case 0xba: case 0xbb: case 0xbc: case 0xbd: case 0xbe: case 0xbf:
			...
			temp = (op >> 18) & 4;
			buffer += snprintf(buffer, 64, "DM(I%d,M%d) = $%04X", temp + ((op >> 2) & 3), temp + (op & 3), (op >> 4) & 0xffff);
			break;
		...
		case 0xf8: case 0xf9: case 0xfa: case 0xfb: case 0xfc: case 0xfd: case 0xfe: case 0xff:
			...
			buffer += snprintf(buffer, 64, "%s = DM(I%d,M%d), %s = PM(I%d,M%d)", dual_xreg[(op >> 18) & 3], (op >> 2) & 3, op & 3,
