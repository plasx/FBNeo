#!/bin/bash
# Build and test a simple Metal demo with ROM loading improvements

echo "==== Building Metal Port Demo with ROM Loading & Frame Pipeline Fixes ===="

# Create build directory
mkdir -p build

# Compile the Metal shader (if present)
if [ -f "src/burner/metal/fbneo_metal_shaders.metal" ]; then
  echo "==== Compiling Metal shaders ===="
  xcrun -sdk macosx metal -c src/burner/metal/fbneo_metal_shaders.metal -o build/fbneo_metal_shaders.air
  xcrun -sdk macosx metallib build/fbneo_metal_shaders.air -o build/fbneo_metal_shaders.metallib
fi

# Create a demo Metal implementation
echo "==== Creating demo implementation ===="
cat > build/metal_demo.mm << 'EOF'
...
# Compile the Metal demo
echo "==== Compiling Metal demo ===="
clang++ -std=c++17 -O2 -fobjc-arc build/metal_demo.mm -o fbneo_metal_demo \
    -framework Cocoa -framework Metal -framework MetalKit \
    -I. -Isrc -Isrc/burn -Isrc/burner
...
if [ -f "fbneo_metal_demo" ]; then
    ...
    echo ""
    echo "To run the Metal demo:"
    echo "./fbneo_metal_demo [path/to/rom.zip]"
    echo ""
    
    # If a ROM path was provided as argument, run it
    if [ "$1" != "" ]; then
        echo "==== Running with ROM: $1 ===="
        ./fbneo_metal_demo "$1"
    else
        echo "No ROM path provided. You can run manually with a ROM path."
    fi
else
    echo "==== Build failed! ===="
    exit 1
